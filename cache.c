...
struct victimInfo {
  ...
  char choseToEvict[6];  //array of bits indicating the policies that chose to evict this block
  char validBit;         //bit indicating this block is valid in the vitim table
...
unsigned int maxHistoryTableSize = 1024;

//func declarations
void addToHistoryTable(struct cache_t* cp, struct cache_blk_t* blk, struct cache_blk_t** choice_arr, md_addr_t blkID);
struct victimInfo * searchHistoryTable (struct cache_t* cp, md_addr_t blkID);
struct cache_blk_t * getLFU(struct cache_set_t *set);
struct cache_blk_t * mruFind(struct cache_set_t *set);
struct cache_blk_t * lruFind(struct cache_set_t *set);
void zeroMRUBits(struct cache_set_t *set);
       ...
       md_addr_t *repl_addr)  /* for address of replaced block */
{
  ...
  md_addr_t blockID = CACHE_BLOCKID(cp, addr);
  ...
  //Traverse history table to search for current block
  struct victimInfo* evictedInfo = searchHistoryTable(cp, blockID);
  if (evictedInfo != NULL) {
  ...
  counter_t lowestScore = 0xffffffff;
  unsigned int chosenPolicy = 0;
  for (int i = 0; i < 6; i++) {
    if (cp->scores[i] < lowestScore) {
      lowestScore = cp->scores[i];
      chosenPolicy = i;
    }
  }

  cp->policyCounts[chosenPolicy]++;

  struct cache_blk_t* choices[6];
  choices[0] = lruFind(&(cp->sets[set])); //LRU
  choices[1] = cp->sets[set].way_tail; //fifo
  choices[2] = mruFind(&(cp->sets[set])); //mru
  int bindex = myrand() & (cp->assoc - 1);

  choices[3] = CACHE_BINDEX(cp, cp->sets[set].blks, bindex); //random
  choices[4] = cp->sets[set].way_head; //lifo
  choices[5] = getLFU(&(cp->sets[set])); //lfu

  struct cache_blk_t* firstNonNull = NULL;
  for (int i = 0; i < 6; i++) {
  	if (choices[i] != NULL && (firstNonNull == NULL)) {
  		firstNonNull = choices[i];
  	}
  }
  ...
  for (int idx = 0; idx < 6; idx++) {
  	if (choices[idx] == NULL) {
  		//cp->resetCount++;
 		//choices[idx] = cp->sets[set].way_head;
 		choices[idx] = firstNonNull;
  	}
  }
  ...
  repl = choices[chosenPolicy];
  if (repl == NULL) {
  	int j = 0;
  }
  //add his info to history table
  md_addr_t replBlockID = (repl->tag) << (((cp)->tag_shift) - ((cp)->set_shift));
  replBlockID = replBlockID | set;
  addToHistoryTable(cp,repl, choices, replBlockID);
...
void addToHistoryTable(struct cache_t* cp, struct cache_blk_t* blk, struct cache_blk_t** choice_arr, md_addr_t blkID) {
  struct victimInfo * newVictim = (struct victimInfo*) malloc(sizeof(struct victimInfo));
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if(cp->historyTableSize == 0){
      cp->historyTableNewest = newVictim;
      cp->historyTableOldest = newVictim;
      newVictim->next = NULL;
      newVictim->prev = NULL;
    }else{
      newVictim->next = cp->historyTableNewest;
      cp->historyTableNewest->prev = newVictim;
      cp->historyTableNewest = newVictim;
      newVictim->prev = NULL;
    }

    if (cp->historyTableSize < maxHistoryTableSize) {
      cp->historyTableSize++;
    } else {
      cp->historyTableOldest = cp->historyTableOldest->prev;
      free(cp->historyTableOldest->next);
      cp->historyTableOldest->next = NULL;
    }
}

struct cache_blk_t * getLFU(struct cache_set_t *set) {
  //Travers set to find block with lowest num_accesses
  unsigned int minNumber = 0xffffffff;
  struct cache_blk_t* minBlk = NULL;
  struct cache_blk_t* current = set->way_head;
  while (current != NULL) {
    if (current->num_accesses < minNumber) {
        minNumber = current->num_accesses;
        minBlk = current;
    }
...
struct victimInfo * searchHistoryTable (struct cache_t* cp, md_addr_t blkID) {
  //
  struct victimInfo* current = cp->historyTableNewest;
  while(current != NULL){
...
struct cache_blk_t * mruFind(struct cache_set_t *set) {
  struct cache_blk_t * current = set->way_tail;
  while (current != NULL) {
    if (current->mruBit == 1) {
      //We return current
      return current;
    } else {
      current = current->way_prev;
    }
  }
  return set->way_tail;
}
...
struct cache_blk_t * lruFind(struct cache_set_t *set) {
  struct cache_blk_t * current = set->way_tail;
  while (current != NULL) {
    if (current->mruBit == 0) { //current->status != 0
      return current;
    } else {
      current = current->way_prev;
    }
...
counter_t* givePolicyCounts() {
	return policyCounts;
}

counter_t* givePolicyScores() {
	return scores;
}
